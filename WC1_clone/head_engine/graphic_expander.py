...
def extract_image(block):
	...
	for i in range(0, dims[0] + dims[1] + 1):
		retimage.image.append(copy(line))
	...
	while o < len(block):
		...
		if keynum % 2 == 0:
			for i in range(0, length):
				if abs(x + dims[1] + i) < len(retimage.image) and abs(y + dims[2]) < len(retimage.image[x + dims[1] + i]) and o+i < len(block):
					retimage.image[x + dims[1] + i][y + dims[2]] = block[o+i]
			o += length
			...
			while added < length and o < len(block):
				if block[o]%2 == 0:
					for i in range(0, block[o]/2):
						if abs(x + dims[1] + i + added) < len(retimage.image) and abs(y + dims[2]) < len(retimage.image[x + dims[1] + i + added]) and o+i+1 < len(block):
							retimage.image[x + dims[1] + i + added][y + dims[2]] = block[o+1+i]
					added += block[o]/2
					o += i+2
				else:
					for i in range(0, block[o]/2):
						if abs(x + dims[1] + i + added) < len(retimage.image) and abs(y + dims[2]) < len(retimage.image[x + dims[1] + i + added]) and o+1 < len(block):
							retimage.image[x + dims[1] + i + added][y + dims[2]] = block[o+1]
					added += block[o]/2
					o += 2
					
	return retimage
	
def display_image(image, size_mult, bright_mult, screen_in = None, x_in = 0, y_in = 24):
	...
	screen2 = pygame.Surface((x, y), SRCALPHA)
	pix = pygame.PixelArray(screen2)
	
	for i in range(0, len(image)):
		for j in range(0, len(image[i])):
			if i < len(pix) and j < len(pix[i]):
				if image[i][j] != None:
					p = [z*bright_mult for z in wc1pal[image[i][j]*3:image[i][j]*3+3]]
	...
	#if no screen was passed, draw this image to a new window, and wait for a keypress
	if screen_in == None:
		...
		while(1):
			if pygame.event.poll().type == pygame.KEYDOWN:
				break
		return screen
	#if a surface was passed, blit the new image to the correct coordinates
	else:
		screen3 = screen2.copy()
		screen_in.blit(screen3, (x_in, y_in))
		return screen_in
	
briefing = readfile("briefing.vga")
for b in briefing[2:3]:
	for i in b:
		x = extract_image(i)
		if len(x.image) > 1:
			#display_image(x.image, 4, 4, None, 0, 0)
			pass