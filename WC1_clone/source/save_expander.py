...
class Savegame:
	...
	def Save(self):
		...
		for p in game_structures.pilots:
			outd += chr(p.diedin%256) + chr(p.diedin/256)
	...
	def Load(self):
		...
		game_structures.Bluehair.bronzestars = ord(self.data[364])
		game_structures.Bluehair.silverstars = ord(self.data[365])
		game_structures.Bluehair.goldstars = ord(self.data[366])
		game_structures.Bluehair.goldsuns = ord(self.data[367])
		game_structures.Bluehair.pewter = ord(self.data[368])
		
		ribbons = [ord(b) for b in self.data[369:381]]
		for r in range(0, len(ribbons)):
			game_structures.Bluehair.ribbons[game_structures.rib_names[r]] = ribbons[r]
		...
		##should compare these 25 bytes to unidentified branching structures and mission flags
